...
if __name__ == '__main__':
  ...
  for label_file, pred_file in zip(label_names, pred_names):
    ...
    u_label_inst_help  = u_label_inst.copy()
    u_label_inst_help[u_label_sem_class == 0] = 0
    u_label_inst_help[u_label_sem_class >= 9] = 0

    false_positive_inds = np.where((u_pred_inst_help > 0) & (u_label_inst_help == 0))
    false_negative_inds = np.where((u_pred_inst_help == 0) & (u_label_inst_help > 0)) #these FN can result from wrong foreground/backround predictions and from wrong clustering
    false_negative_inds_clust = np.where((u_pred_inst_help == 0) & (u_label_inst_help > 0) & (u_pred_sem_class < 9)) #these FN only result from wrong clustering
    ...
    instances_gt = np.unique(u_label_inst_help)
    ...
    for instance in np.nditer(instances_gt):
      ...
      if corresponding_pred_instances.shape[0] > 1:
        ...
        for i in np.nditer(corresponding_pred_instances):
          ids = np.where((u_pred_inst_help == i) & (u_label_inst_help == instance))[0]
          n = ids.shape[0]
          dict[int(i>>16)] = n
        do_print = True
        for key in dict:
          if dict[key] < 500:
            do_print = False
        if do_print:
          print(label_file + " Instance: " + str(instance>>16) + " (" + str(instance_ids.shape[0]) + ") " + ", divided into " + str(corresponding_pred_instances.shape[0]) + ", " + str(dict))
    ...
    for instance in np.nditer(instances_pred):
      ...
      corresponding_gt_instances = np.unique(u_label_inst_help[instance_ids])
      zero = np.array([0])
  ...
  #IDR,IDP,IDF1 = class_evaluator.getID()
  ...
  #print("IDR things: ", IDR[1:9])
  #print("mIDR things: ", IDR[1:9].mean())

  #print("IDP  things: ", IDP[1:9])
  #print("mIDP things: ", IDP[1:9].mean())

  #print("IDF1  things: ", IDF1[1:9])
  #print("mIDF1 things: ", IDF1[1:9].mean())
