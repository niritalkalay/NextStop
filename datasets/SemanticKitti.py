...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True, seqential_batch = False):
        ...
        if self.set == 'training':
            ...
            self.sequences = ['{:02d}'.format(i) for i in range(11, 22)]# if i == 12] # nirit to run only 11 (11,12 donr)
            print(self.sequences)
        else:
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set == 'test':
                    ...
                    center_file = join(seq_path, 'labels', self.frames[s_ind][f_ind - f_inc] + '.labels-4d-stop.center.npy')
                # Read points
                ...
                if self.set in ['validation', 'test'] and self.config.n_test_frames > 1 and f_inc > 0:
                    ...
                    test_path = join(self.config.test_path,self.config.saving_path.split('/')[-1] + '_' + self.config.assoc_saving + str(self.config.n_test_frames) + '_' + self.config.chosen_chkp)
                    ...
                    if self.config.sampling == 'objectness':
                        ...
                        #print('self.config.sampling == ','objectness', ' :  filename', filename)
                        ...
                        file_path = join(test_path, filename)
                        #print('self.config.sampling == ', self.config.sampling, ' :  file_path', file_path) nirit
                        center_pred = None
                        counter = 0
