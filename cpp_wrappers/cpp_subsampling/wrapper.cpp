...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	PyObject* ins_classes_obj = NULL;
	PyObject* batches_obj = NULL;

	// Keywords containers
	static char* kwlist[] = { "points", "batches", "features", "classes", "ins_classes", "sampleDl", "method", "max_p", "verbose", NULL };
	float sampleDl = 0.1;
	...
	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|$OOOfsii", kwlist, &points_obj, &batches_obj, &features_obj, &classes_obj, &ins_classes_obj, &sampleDl, &method_buffer, &max_p, &verbose))
	{
	...
	PyObject* ins_classes_array = NULL;
	if (use_feature)
	...
	if (use_classes){
		...
		ins_classes_array = PyArray_FROM_OTF(ins_classes_obj, NPY_INT, NPY_IN_ARRAY);
    }
	...
	// Create pyramid
	if (verbose > 0)
		cout << "Computing cloud pyramid with support points: 1" << endl;
	...
	vector<int> original_ins_classes;
	...
	if (use_classes){
		...
		original_ins_classes = vector<int>((int*)PyArray_DATA(ins_classes_array), (int*)PyArray_DATA(ins_classes_array) + N * ldim);
    }
	// Subsample
	vector<PointXYZ> subsampled_points;
	vector<float> subsampled_features;
	vector<int> subsampled_classes;
	vector<int> subsampled_ins_classes;
	...
	batch_grid_subsampling(original_points,
							...
							original_classes,
							subsampled_classes,
                            original_ins_classes,
							subsampled_ins_classes,
							original_batches,
							subsampled_batches,
							sampleDl,
							max_p);
	...
	PyObject* res_ins_classes_obj = NULL;
	PyObject* ret = NULL;
	...
	if (use_feature && use_classes)
		ret = Py_BuildValue("NNNNN", res_points_obj, res_batches_obj, res_features_obj, res_classes_obj, res_ins_classes_obj);
	else if (use_feature)
		ret = Py_BuildValue("NNN", res_points_obj, res_batches_obj, res_features_obj);
	else if (use_classes)
		ret = Py_BuildValue("NNNN", res_points_obj, res_batches_obj, res_classes_obj, res_ins_classes_obj);
	else
...
static PyObject* cloud_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Keywords containers
	static char* kwlist[] = { "points", "features", "classes", "ins_classes", "sampleDl", "method", "verbose", NULL };
	float sampleDl = 0.1;
	const char* method_buffer = "barycenters";
	int verbose = 0;

	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|$OOOfsi", kwlist, &points_obj, &features_obj, &classes_obj, &ins_classes_obj, &sampleDl, &method_buffer, &verbose))
	{
		PyErr_SetString(PyExc_RuntimeError, "Error parsing arguments");
		return NULL;
	}
	...
	// Create pyramid
	if (verbose > 0)
		cout << "Computing cloud pyramid with support points: 2" << endl;
	...
	// Merge results
	if (use_feature && use_classes)
		ret = Py_BuildValue("NNNN", res_points_obj, res_features_obj, res_classes_obj, res_ins_classes_obj);
	else if (use_feature)
		ret = Py_BuildValue("NN", res_points_obj, res_features_obj);
	else if (use_classes)
		ret = Py_BuildValue("NNN", res_points_obj, res_classes_obj, res_ins_classes_obj);
	else
