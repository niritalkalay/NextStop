...
class Box3D:
    def __init__(self, x=None, y=None, z=None, h=None, w=None, l=None, ry=None, c=None):
        self.x = x  # center x
        self.y = y  # center y
        self.z = z  # center z
        self.h = h  # height (z)
        self.w = w  # width (y)
        self.l = l  # length  (x)
        self.ry = ry  # orientation (radians)
        self.c = c # class label
        self.corners_3d_cam = None

    def __str__(self):
        return 'x: {}, y: {}, z: {}, heading: {}, length: {}, width: {}, height: {}, class: {}'.format(
            self.x, self.y, self.z, self.ry, self.l, self.w, self.h, self.c)
    ...
    def bbox2array_raw(cls, bbox):
        if bbox.c is None:
            ...
            return np.array([bbox.h, bbox.w, bbox.l, bbox.x, bbox.y, bbox.z, bbox.ry, bbox.c])
