...
def findClosest(points, ind1, ind2, threshold=1):
...
def getIndToPointsInsideBoxes(array_of_boxes, points, label_sem_class, label_inst, array_of_ClassID):
...
def getIndToPointsInsideBoxes_(array_of_boxes, points, label_sem_class, label_inst):
    results = [None] * len(array_of_boxes)
...
def init(sequence, split, path_to_gt, seg_prediction_dir, box_tracker_path):
    ...
    # new path
    parent_path = os.path.join(box_tracker_path, "sequences", "{0:02d}".format(sequence))
    path_to_track_result = os.path.join(parent_path, "predictions", "trk_withid_0")
    save_path = os.path.join(parent_path, 'to_labels')
...
def main(args):
    ...
    for sequence in test_sequences:
        ...
        save_path, scan_names, prediction_path, track_names, config_data = init(sequence,
                                                                                args.split,
                                                                                args.dataset,
                                                                                args.predictions,
        ...
        classStr2Int_local = {config_data['labels'][value]: key for key, value in
                              config_data['learning_map_inv'].items()}  #
        classInt2Str_local = {key: config_data['labels'][value] for key, value in config_data[
            'learning_map_inv'].items()}  # lables match 4dStop net learning, called it in local map
        ...
        for idx in range(len(scan_names)):
            ...
            # if idx !=289: #2772:#3006:  # to debug  filling close objects
            #  continue
            ...
            #  continue
            # if idx != 72:#"#!= 13:#3965:#3976:#149:#3965:#2612:#3999:#3973:#3999:#3965:
            #  continue
            ...
            array_of_boxes, array_of_TrackedID, array_of_ClassID, array_of_DetectionScore = parse_boxes(
                filename=track_file)
            ...
            list_of_ind_points_inside_box = getIndToPointsInsideBoxes(array_of_boxes, points, label_sem_class,
                                                                      label_inst,
                                                                      array_of_ClassID)
            ...
            for b in range(len(array_of_boxes)):
                ...
                if len(unmarked_points_idx != 0):
                    object_index = list_of_ind_points_inside_box[
                        b]  # getPointsInsideBox(box,points[unmarked_points_idx])
                ...
                # draw_scenes_raw(points=points[object_index,:], gt_boxes=[box], title='getPointsInsideBox : ')
            ...
            # print("")
            # label_test = np.fromfile(filename, dtype=np.uint32)
        ...
        with open(filename, 'a') as the_file:
            ...
            for cls_group_ind_in_mem in range(3):
                for ii in range(len(IDMemory[cls_group_ind_in_mem].trk_id)):
                    txt = str(cls_group_ind_in_mem) + " , " + str(
                        IDMemory[cls_group_ind_in_mem].trk_id[ii]) + " , " + str(
                        IDMemory[cls_group_ind_in_mem].label_id[ii])
                    the_file.write(txt + '\n')
    ...
    with open(filename, 'a') as the_file:
        ...
        for cls_group_ind_in_mem in range(3):
            for ii in range(len(IDMemory[cls_group_ind_in_mem].trk_id)):
                txt = str(cls_group_ind_in_mem) + " , " + str(IDMemory[cls_group_ind_in_mem].trk_id[ii]) + " , " + str(
                    IDMemory[cls_group_ind_in_mem].label_id[ii])
                the_file.write(txt + '\n')
