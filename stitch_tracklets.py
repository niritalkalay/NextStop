...
ignore_size = 25 # orig 25
track_size  = 30 # orig 50
...
def associate_instances_overlapping_frames(previous_ins_label, current_ins_label):
    ...
    previous_instance_ids = [x for i,x in enumerate(previous_instance_ids) if c_p[i] > ignore_size] #
    current_instance_ids = [x for i, x in enumerate(current_instance_ids) if c_c[i] > track_size] #
...
def main(FLAGS):
    ...
    save_path = '{}/stitch'.format(prediction_dir) + assoc_saving +'_ignore{}'.format(ignore_size)+'_track{}'.format(track_size)
    ...
    for poses_seq, sequence in zip(poses, test_sequences):
        print("processing seq ", '{0:02d}'.format(sequence))
        point_names = []
        ...
        for idx, point_file in zip(range(len(point_names)), point_names):
            ...
            for ins_id in np.unique(ins_preds):
                ...
                if valid_ind.shape[0] < ignore_size:
