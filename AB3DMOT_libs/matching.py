...
from AB3DMOT_libs.dist_metrics import iou, dist3d, dist_ground, m_distance,PointsSimilarity,getPointsInsideBox

def compute_affinity(dets, trks, metric, trk_inv_inn_matrices=None,points=None):
	...
	for d, det in enumerate(dets):
		for t, trk in enumerate(trks):
			...
			elif metric == 'PointsSimilarity':
				det_points= getPointsInsideBox (points,det)
				trk_points = getPointsInsideBox(points, trk)
				dist_now = PointsSimilarity(det_points, trk_points)
				#print(dist_now)
			else: assert False, 'error'
			aff_matrix[d, t] = dist_now
			#print('(d,t,aff_matrix[d, t] ) = (',d,t,aff_matrix[d, t] ,')')
	return aff_matrix
...
def data_association(dets, trks, metric, threshold, algm='greedy', \
	trk_innovation_matrix=None, hypothesis=1,points=None):
	"""
	...
	# compute affinity matrix
	aff_matrix = compute_affinity(dets, trks, metric, trk_inv_inn_matrices,points)
...
def refine_detection(dets, unmatched_dets, trks,threshold):
	# inaccurate detections caused by two or more spatially close objects
	# might increase the object’s identity switch and false alarms
	# so remove the close false detection
	refined_unmatched_dets = []
	idx_to_remove =[]
	if len(unmatched_dets) != 0 and len(trks)!=0:
		for d, det in enumerate(dets):
			if d in unmatched_dets:
				for t, trk in enumerate(trks):
					dist_now = iou(det, trk, 'diou_3d')
					#print("dind,tID,dist_now = ", d, t, dist_now)
					if dist_now > threshold:
						# found overlap box
						idx_to_remove.append(d)
						break

		if len(idx_to_remove)!=0:
			refined_unmatched_dets = [i for i in unmatched_dets if i not in idx_to_remove]
		else:
			refined_unmatched_dets = unmatched_dets
	else:
		refined_unmatched_dets = unmatched_dets

	return refined_unmatched_dets
