...
from Xinshuo_PyToolbox_master.xinshuo_io import mkdir_if_missing, load_txt_file, save_txt_file
...
def get_saving_dir(eval_dir_dict, seq_name, save_dir, num_hypo):
	...
	for index in range(num_hypo):
		...
		save_trk_dir[index] = os.path.join(save_dir, 'trk_withid_%d' % index); mkdir_if_missing(save_trk_dir[index])
...
def save_results(res, save_trk_file, eval_file, det_id2str, frame, score_threshold):
	...
	if conf_tmp >= score_threshold and eval_file is not None:
